...
# MAYBE upon click of row, highlight that data point in the plot, if open. 
# 
# History:
#
# Version 0.5:
...
# Improved structure of summary window, it's now a table generated by a loop.
#
# Version 0.3:
# Fixed some references that were broken with strftime() and strptime()
# Implement "Save before closing?" dialog TODO
...
class PyMPG:
    ...
    def createEditWindow(self, row):
        ...
        for x in range(0, len(self.storedFields)):
            ...
            if (self.storedFields[x] == 'fill'):
                ...
                # the "changed" signal is a bit too terse
                # maybe focus-out-event is what I want
                # TODO removed connection to "changed" and implement 
                # a better destruction method for the edit window
                entry.connect("changed", self.updateField, row, x)
                table.attach(entry, 1, 2, x, x+1)
    ...
    def getColRangeSum(self, field, start, stop):
        col = self.storedFields.index(field)
        sum = 0
