...
class PyMPG:
    ...
    def on_row_select(self, widget):
        ...
        if (self.gnuplot_p != False): # gnuplot pipe is open
            if (self.getText(row,self.plot_type) == invalidStr):
                ...
                self.gnuplot_annot = ", \"< echo %d %s\" using 1:2 with points lt 3 pt 3" % (time.mktime( self.recordTable[row][self.storedFields.index('date')].timetuple() ), self.getText(row, self.plot_type) )
    ...
    def plotData(self, field):
        ...
        for x in range(0, len(self.recordTable)):
            if (not (field == "mpg" and not self.recordTable[x][self.storedFields.index('fill')] )):
                # Convert the datetime obj to Epoch seconds
    ...
    def plotSave(self, widget):
        ...
        dialog.set_current_name(self.filename.replace('.csv', '.ps'))
	...
	dateCol = self.storedFields.index('date')
    ...
    def createEditWindow(self, row):
        ...
        for x in range(0, len(self.storedFields)):
            ...
            if (self.storedFields[x] == 'fill'):
                ...
                entry.connect("focus-out-event", self.editWindowEntryFocusOut, 
                    row, x)
                table.attach(entry, 1, 2, x, x+1)
    ...
    def createSummaryWindow(self):
        ...
        dateCol = self.storedFields.index('date')
        odoCol = self.storedFields.index('odo')
    ...
    def editWindowEntryFocusOut(self, widget, event, row, col):
        return self.updateField(widget, row, col)

    def updateField(self, entry, row, col):
        if (entry.get_text() != self.getText(row, self.storedFields[col])):
            self.makeDirty()
            ...
            try:
                ...
                self.newstatus("Updated %s on record %d" 
                    % (self.storedFieldLabels[col],row))
                self.window.queue_draw()
            except ValueError:
